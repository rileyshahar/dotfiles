...
function runCommand(cmd: string, args: string[], input?: string): Promise<{ code: number; stdout: string; stderr: string }> {
...
export function Picker({ command }: { command: string }) {
  ...
  async function run(hovered: string) {
    ...
    await showToast({ style: Toast.Style.Animated, title: `Running on ${lines.length} item${lines.length === 1 ? "" : "s"}…` });
    try {
      if (config.sink_mode === "each") {
        const results = await Promise.all(lines.map((l) => runCommand(sink, [l])));
        const failures = results.filter((r) => r.code !== 0);
        if (failures.length > 0) {
          showToast({ style: Toast.Style.Failure, title: `sink failed for ${failures.length}/${lines.length}`, message: failures[0].stderr.trim() });
          return;
        }
      } else {
  ...
  return (
    <List isLoading={loading} searchBarPlaceholder={config.placeholder}>
      {items.map((line) => (
        <List.Item
          key={line}
          title={line}
          ...
          actions={
            <ActionPanel>
              <Action title={`Run on ${runCount(line)} Item${runCount(line) === 1 ? "" : "s"}`} icon={Icon.Play} onAction={() => run(line)} />
              <Action title="Toggle Selection" icon={Icon.CheckCircle} shortcut={{ modifiers: ["shift"], key: "return" }} onAction={() => toggle(line)} />
            </ActionPanel>
          }
        />
      ))}
